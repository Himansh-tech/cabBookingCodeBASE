...
import {
  ...
  MapPin,
  CheckCircle,
  XCircle, // Import XCircle for "Not Approved" status
  Info, // Import Info for general information or secondary status
  User,
} from "lucide-react";
...
const DriverDashboard = () => {
  ...
  // Function to fetch payment details for a single ride
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    setError(null);
    setPaymentLoading({}); // Reset payment loading state on new fetch
    ...
    try {
      if (!driver || !driver.userId) {
        ...
        logout(); // Ensure logout is called to clear invalid session
        navigate("/login");
        return;
      }
      ...
      const DRIVER_PROFILE_API_URL = `${API_GATEWAY_URL}/api/v1/drivers/${driverId}`;
      ...
      const DRIVER_RIDES_API_URL = `${API_GATEWAY_URL}/api/v1/drivers/rideHistory/${driverId}`;
      ...
      // Updated filter for pending rides to include RIDE_STARTED
      ...
      const completed = await Promise.all(
        ridesData
          ...
          .map(async (ride) => {
            ...
            // Merge payment status and amount into the ride object
            return {
              ...ride,
              paymentStatus: paymentData?.status,
              actualFare: paymentData?.amount || ride.actualFare, // Prefer payment amount if available
            };
          })
      );
      ...
      // --- Calculate Total Earnings based on SUCCESSFUL payments ---
      setTotalEarnings(
        completed.reduce((sum, ride) => {
          // Only add to total earnings if ride is completed AND payment is SUCCESS
  ...
  }, [driver, token, logout, navigate, fetchPaymentDetails]); // Dependencies for useCallback

  useEffect(() => {
    // Only call fetchData if token is available
    if (token) {
      fetchData();
    } else {
      setLoading(false);
      setError("Not authenticated. Please log in to view dashboard.");
      // navigate("/login"); // Redirect to login if token is missing on mount
    }
  }, [token, driver, fetchData, navigate]); // Add fetchData to useEffect dependencies

  // Memoize handleAcceptRide
  const handleAcceptRide = useCallback(async () => {
    ...
    try {
      ...
      // Re-fetch data to update the dashboard after a successful action
      fetchData();
    } catch (error) {
      console.error("Failed to accept ride:", error);
      alert(`Failed to accept ride: ${error.message || "Unknown error"}`);
    }
  }, [driver, token, fetchData]); // Add fetchData to dependencies

  // Memoize handleCompleteRide
  const handleCompleteRide = useCallback(
    async (rideId) => {
      ...
      try {
        ...
        alert("Ride completed! Waiting for user payment."); // Adjusted alert message
        // Re-fetch data to update the dashboard after a successful action
        fetchData();
      } catch (error) {
  ...
  const totalRidesCount = completedRides.length + pendingRides.length; // Still counts all in progress/completed
      ...
      <Row className="mb-4 g-4">
        ...
        <Col xs={12} md={6} lg={4}>
          <Card className="p-4 shadow-sm rounded-3">
            <Card.Body className="d-flex align-items-center justify-content-between p-0">
              <div>
                <p className="small fw-medium text-muted mb-1">Total Rides</p>
                  ...
                  <Col xs={3} className="text-end">
                    {ride.status === "DRIVER_ASSIGNED" && driverProfile.approved && (
                      <Button
                        ...
                        disabled={!driverProfile.approved} // Disable if not approved or already on a ride
                      >
                        <CheckCircle size={16} className="me-2" />
                        <span>Accept</span>
                      </Button>
                    )}
                    {ride.status === "RIDE_STARTED" && driverProfile.approved && (
                      <Button
                        variant="info"
                        size="sm"
                        className="d-inline-flex align-items-center py-2 px-3 rounded-lg fw-medium"
                      ...
                      <div className="d-flex align-items-center mb-2">
                        {paymentLoading[ride.id] ? (
                          ...
                          >
                            ...
                            {ride.status === "COMPLETED" && (ride.paymentStatus === "PENDING" || ride.paymentStatus === "NOT_FOUND") && " (WAITING FOR PAYMENT)"}
