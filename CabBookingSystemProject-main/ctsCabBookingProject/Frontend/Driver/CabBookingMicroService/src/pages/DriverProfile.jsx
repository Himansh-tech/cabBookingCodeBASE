...
import { User, Mail, Phone, Edit3, Save, X, Car, CheckCircle, XCircle } from "lucide-react"; // Import new icons
import {
  Container,
  Row,
  Col,
  ...
  Spinner,
  Alert,
  Badge, // Import Badge for consistent status display
...
const DriverProfile = () => {
  ...
  // Wrap fetchDriverProfile in useCallback
  ...
  }, [driver, token, logout, navigate]); // Dependencies for useCallback: driver, token, logout, navigate

  // UseEffect to call the memoized fetchDriverProfile
  useEffect(() => {
  ...
  }, [token, driver, fetchDriverProfile]); // Now, fetchDriverProfile is a stable dependency

  // Wrap handleSave in useCallback
  ...
  }, [driver, token, logout, navigate, formData, fetchDriverProfile]); // formData is a dependency here

  // Wrap handleStatusChange in useCallback
  const handleStatusChange = useCallback(async (selectedStatusKey) => {
    if (!driverData.approved) {
      ...
      return; // Prevent status change if not approved
    }
    ...
    try {
      ...
      const driverId = driver.userId;
      // Map UI friendly key back to backend enum
      let backendStatus;
      if (selectedStatusKey === "Available") {
        ...
        // Handle unexpected status key, though dropdown should prevent this
      ...
      // For a PATCH request with query parameters, the body is often empty or minimal
      await axios.put(STATUS_API_URL, {}, { // Empty object for body as status is in query param
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      alert(`Status changed to ${getDisplayStatus(backendStatus)}!`); // Use getDisplayStatus for alert
      fetchDriverProfile(); // Re-fetch to get the latest data and update UI
    } catch (err) {
  ...
  }, [driver, token, logout, navigate, fetchDriverProfile, getDisplayStatus, driverData]); // Dependencies for useCallback including driverData for `approved` status

  // Handler for canceling edit (no API call, so no useCallback strictly needed, but good practice)
  const handleCancel = () => {
    if (driverData) {
      setFormData({
  ...
  // Handler for form field changes (no API call, doesn't need useCallback)
  ...
  // Derive current status for display
  ...
  const isDriverApproved = driverData.approved; // Get the approved status
            ...
            <div className={`p-3 rounded-3 ${isStatusActive ? "bg-info-subtle" : "bg-warning-subtle"}`}>
              ...
              <Dropdown onSelect={handleStatusChange}>
                <Dropdown.Toggle
                  variant="secondary"
                  id="dropdown-status"
                  // Disable toggle if driver is not approved
                  disabled={!isDriverApproved}
                >
                  Change Status
                </Dropdown.Toggle>
                <Dropdown.Menu>
                  {/* Disable dropdown items based on ON_RIDE status or if not approved */}
                  <Dropdown.Item
                    eventKey="Available"
