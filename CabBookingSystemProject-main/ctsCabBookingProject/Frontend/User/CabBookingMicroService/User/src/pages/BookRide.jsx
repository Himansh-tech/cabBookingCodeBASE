...
const rideTypes = [
  ...
  { id: "luxury", label: "Luxury", multiplier: 2 },
];

const BookRide = () => {
  const { token, user, isBookRideDisabled } = useAuth(); // Get token, user, AND isBookRideDisabled from AuthContext
  ...
  const handleRouteInfo = ({ distance, time }) => {
    ...
    setDuration(time);
    setActualFare(fare); // Corrected typo
  ...
  const handleBookRide = async () => {
    ...
    // Ensure all values are numeric and positive before booking
    ...
    const requestPayload = {
      ...
      duration: duration, // Send as number
    ...
    try {
      ...
      setActualFare(0); // Corrected typo
      setDistance(0);
      setDuration(0);
      setShowEstimate(false);
      setRideType("economy");
  ...
  // If isBookRideDisabled is false, render the actual booking form
  return (
      ...
      <Card className="p-4 shadow-lg rounded-4 mb-4">
        <Card.Body>
          ...
          <Form className="mb-4">
            ...
            <Form.Group className="mb-3" controlId="rideType">
