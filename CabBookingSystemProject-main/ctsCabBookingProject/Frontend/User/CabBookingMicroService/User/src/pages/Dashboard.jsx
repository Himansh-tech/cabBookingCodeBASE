...
const Dashboard = () => {
  ...
  const fetchPaymentStatus = useCallback(async (rideId) => {
    ...
    try {
      const response = await axios.get(`${API_GATEWAY_URL}/api/v1/payments/ride/${rideId}`, {headers: {'Authorization': `Bearer ${token}`}});
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Process rides to fetch payment status for completed rides
        const ridesWithPaymentStatus = await Promise.all(
          fetchedRides.map(async (ride) => {
            ...
            return ride; // For non-completed rides return as it is
          })
        );

        setRides(ridesWithPaymentStatus);//give value to ride
        setLoading(false);
    ...
    if (token && user?.userId) { // Ensure user.userId is available before fetching
      fetchData();
    } else {
      setLoading(false);
    ...
    <Container className="my-4 py-4">
      <Row className="mb-4 justify-content-between align-items-center">
        ...
        <Col xs={12} md={4} className="text-md-end mt-3 mt-md-0">
          <Link
            to="/book-ride"
            className={`btn btn-primary d-inline-flex align-items-center py-2 px-3 rounded-lg fw-semibold shadow-sm `}
            style={{
      ...
      <Row className="mb-4 g-4">
        <StatCard
          label="Completed"
          value={rides.filter((r) => r.status === "COMPLETED" && r.paymentStatus === "SUCCESS").length} // Count only truly completed and paid
        ...
        {/* <StatCard
          label="Cancelled"
          value={rides.filter(r => r.status === "CANCELLED_BY_USER" || r.status === "CANCELLED_BY_DRIVER" || r.status === "NO_DRIVER_AVAILABLE").length}
          icon={<Car size={24} className="text-danger" />}
          bg="danger"
        /> */}
      </Row>
              ...
              >
                <Row className="align-items-start">
                  <Col xs={9}>
                    ...
                    {(ride.status === "DRIVER_ASSIGNED" || ride.status === "RIDE_STARTED" || ride.status === "COMPLETED") && ride.driverFullname && (
                  ...
                  <Col xs={3} className="text-end d-flex flex-column align-items-end justify-content-between">
                    <Badge
                      className={`px-2 py-1 rounded-pill text-uppercase fw-semibold ${
                        ride.status === "COMPLETED"
                          ? (ride.paymentStatus === "PENDING" ? "bg-info text-dark" : "bg-success text-white") // Changed for pending payment
                    ...
                    {/* Show spinner while fetching payment status for this ride */}
                    {ride.status === "COMPLETED" && paymentLoading[ride.id] ? (
  ...
  <Col xs={12} md={6} lg={3}>
    <Card className="p-4 shadow-sm rounded-3">
      <Card.Body className="d-flex align-items-center justify-content-between p-0">
        <div>
          <p className="small fw-medium text-muted mb-1">{label}</p>
          <p className="fs-4 fw-bold text-dark mb-0">{value}</p>
        </div>
        <div className={`bg-${bg}-subtle p-3 rounded-circle`}>{icon}</div>
      </Card.Body>
    </Card>
  </Col>
);
